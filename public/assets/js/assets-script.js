( function( $ ) {
	...
	var isEnvatoElMain = false;
	if ( location.href.indexOf('envato-elements') != -1 ) {
		if ( getCookie('isEnvatoEl') == null ) {
			setCookie('isEnvatoEl', 'yes');
		}
		isEnvatoElMain = true;
	} else {
		isEnvatoElMain = false;
	}

  function bslthms_init(){

    var buttons = $('.bsl-popup__buttons');

    var help = $('.bsl-popup__content-help');

    var content = $('.bsl-popup__content-related');
    var content_menu = $('.bsl-popup__content .bsl-popup__menu');
    var content_related = $('.bsl-popup__content .bsl-popup__tab-related');
    var content_all = $('.bsl-popup__content .bsl-popup__tab-all');
    var content_demo = $('.bsl-popup__content .bsl-popup__tab-demo');
		var content_version = $('.bsl-popup__content .bsl-popup__tab-version');

    var theme = $('.bsl-popup').data('theme');
    var category = $('.bsl-popup').data('category');

    var itemDetails = false;
    var itemsList = false;
		var itemsAllList = false;
    var renderedDemo = false;
    var renderedRelated = false;
    var renderedAll = false;
		var renderedVersion = false;
      ...
      setTimeout(function(){
        $.ajax({
          ...
          success: function(data) {
						itemsAllList = data.items;

            $.each( data.items, function(key, type) {
              if ( key == category ) {
                itemsList = type;

                $.each( type, function(item_key, item_value) {
                  if ( item_key == theme ) {
                    itemDetails = item_value;
                    renderButtons(buttons, itemDetails, itemsAllList, category, theme);
                  }
                });
              }
            });

            renderContentMenu(content_menu, itemDetails, category);
          },
    ...
    $(document).on( 'click', '#relatedThemes', function() {
      if ( $(this).hasClass('active') ) {
        content.parent().removeClass('opened');
        $(this).removeClass('active');
      } else {
        $(this).parent().find('a:not(#relatedThemes)').removeClass('active');
        $(this).addClass('active');

        content.parent().find('.bsl-popup__content').removeClass('active');
        content.addClass('active');

        if ( !content.parent().hasClass('opened') ){
          content.parent().addClass('opened');
        }

        if ( itemDetails.demo ) {
          $('.bsl-popup__menu a[href="#tab-demo"]').trigger('click');
        } else {
          $('.bsl-popup__menu a[href="#tab-related"]').trigger('click');
        }
      }
    ...
    $(document).on( 'click', '#helpThemes', function() {
      if ( $(this).hasClass('active') ) {
        help.parent().removeClass('opened');
        $(this).removeClass('active');
      } else {
        $(this).parent().find('a:not(#helpThemes)').removeClass('active');
        $(this).addClass('active');

        help.parent().find('.bsl-popup__content').removeClass('active');
        help.addClass('active');

        if ( !help.parent().hasClass('opened') ){
          help.parent().addClass('opened');
        }

        renderContentHelp(help, itemDetails);
      }

      return false;
    });

    $(document).on( 'click', '#closeBslpopup', function() {
    ...
    $(document).on( 'click', '.bsl-popup__menu a', function() {
      $(this).parent().find('a').removeClass('active');
      $(this).addClass('active');

      var id = $(this).attr('href').replace('#tab-', '');

      $('.bsl-popup__tab').removeClass('active');
      $('.bsl-popup__tab-'+id).addClass('active');

      if ( id == 'demo' ) {
        if ( !renderedDemo ) {
          renderedDemo = true;
          renderDemoContent(content_demo, itemDetails, theme);
        }
      }
      if ( id == 'related' ) {
        if ( !renderedRelated ) {
          renderedRelated = true;
          renderRelatedContent(content_related, itemsList, itemDetails, theme);
        }
      }
      if ( id == 'all' ) {
        if ( !renderedAll ) {
          renderedAll = true;
          renderAllContent(content_all, itemsList, itemDetails, theme);
        }
      }
			if ( id == 'version' ) {
        if ( !renderedVersion ) {
          renderedVersion = true;
          renderVersionContent(content_version, itemsAllList, theme);
        }
      }
    ...
    $(document).on( 'click', '#hideBslpopup', function() {
      $('.bsl-popup').remove();

      setCookie('hideBslPopup', 'yes');
  ...
  function renderContentMenu(content, itemDetails, category){
    var html = '';

    if ( itemDetails.demo ) {
      html += '<a href="#tab-demo" class="active">Demo</a><a href="#tab-related">Related</a>';
    } else {
      html += '<a href="#tab-related" class="active">Related</a>';
    }

    html += '<a href="#tab-all">All</a>';

		if ( category == 'html' || category == 'jekyll' || category == 'hugo' ) {
			html += '<a href="#tab-version">Version</a>';
		}

    content.html(html);
  }

  function renderRelatedContent(content, itemsList, itemDetails, theme) {
    var results = [];
    ...
    $.each( itemsList, function(key, value) {
      if ( key != theme ) {
        $.each( tags, function(tag_key, tag_value) {
          if ( value.tags.includes( tag_value ) ){
            if ( results.findIndex( x => x.slug == key ) === -1 ){
              results.push({slug:key, item:value});
            }
          }
        });
      }
    ...
    $.each( results, function(key, value) {
      html += '<li><a href="'+ value.item.links.url +'" target="_blank"><img src="'+ value.item.preview +'" alt="'+ value.item.title +'" /><span>' + value.item.title + '</span></a></li>';
    } );
  ...
  function renderAllContent(content, itemsList, itemDetails, theme) {
    var results = [];
    var tags = itemDetails.tags.split(',');
    var html = '<ul>';
  ...
  function renderDemoContent(content, itemDetails, theme) {
    var html = '<ul>';

    $.each( itemDetails.demo, function(key, value) {
      html += '<li><a href="'+ value.url +'"><img src="'+ value.preview +'" alt="'+ value.title +'" /><span>' + value.title + '</span></a></li>';
    } );
	...
	function renderVersionContent(content, itemsAll, theme) {
		var versionList = [];

		$.each( itemsAll, function(key, type) {
			$.each( type, function(item_key, item_value) {
				if ( item_key == theme ) {
					versionList.push({ slug:key, item:item_value });
				}
			});
		});
    ...
    $.each( versionList, function(key, value) {
      html += '<li><a href="'+ value.item.links.url +'" target="_blank"><img src="'+ value.item.preview +'" alt="'+ value.item.title +'" /><span>' + value.item.title + ' ['+value.slug+']'+'</span></a></li>';
    } );
